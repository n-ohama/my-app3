...
export default function PostArticle({post, user_id}: {post: any, user_id: string}) {
  ...
  return (
    <article 
      ...
      className="rounded-2xl border border-slate-800 bg-slate-950/70 p-4 cursor-pointer"
    >
      ...
      <div className="flex gap-6 text-sm text-slate-400">
        <button
          type="button"
          className="transition hover:text-slate-200 cursor-pointer"
          onClick={(e) => {
            e.stopPropagation();
            alert("comment dialog should be shown up")
          }}
        >
          💬 {post.stats?.comments}
        </button>
        ...
        <button
          type="button"
          className="transition hover:text-slate-200 p-2 cursor-pointer"
          onClick={async (e) => {
            e.stopPropagation();
